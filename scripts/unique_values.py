...
class Uniquer:
    ...
    def write_out(self):
        ...
        dest_file = open(
            r"{}/UNIQUE_VALUES.txt".format(self.destination), "w+")
    ...
    def run(self):
        ...
        if self.group_by:
            if self.group_for is not None:
                for col in self.group_for:
                    self.unique_frames.append(('group_by: {},group_for: {}'.format(
                        self.group_by, col), self.raw_data.groupby(self.group_by)[col].unique()))
            else:
...
def cli():
    my_parser = argparse.ArgumentParser(description="unique_values is an upgraded version of the UniqueCols utility used to find all the unique values in each column of a CSV or XLSX",
    ...
    my_parser.add_argument('-d', '--destination', required=False,
                           type=str, help="The path to the destination directory, the output will be written here (default is input_file directory)")
    ...
    my_parser.add_argument(
        '-e', '--encoding', required=False, default='ISO-8859-1', help="Override the encoding interpretation of the passed in file (default is ISO-8859-1, utf8 is another option)")
